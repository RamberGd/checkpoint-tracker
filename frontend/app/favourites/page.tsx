...
export default function FavouritesPage() {
  ...
  // 'favourites' maps to played games flagged is_favorite on the backend.
  useEffect(() => {
    let active = true;
    api
      .list("favourites")
      ...
      .catch((err) => {
        ...
        else console.error("Failed to load favourites", err);
      })
    ...
    <RequireAuth>
      ...
      <main className={styles.main}>
        {!loading && games.length === 0 ? (
          <EmptyShelf line="The ones worth returning to — when you find them." />
        ) : (
          rows.map((row, rowIdx) => (
