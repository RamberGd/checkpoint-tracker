...
export default function PlayedPage() {
  ...
  // Load the full played list (all entries, not the 5-item profile shortlist).
  useEffect(() => {
    let active = true;
    api
      .list("played")
      ...
      .catch((err) => {
        ...
        else console.error("Failed to load played list", err);
      })
    ...
    <RequireAuth>
      ...
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <h1 className={styles.heading}>Played</h1>
      ...
      <main className={styles.main}>
        {!loading && games.length === 0 ? (
          <EmptyShelf line="The archive is empty. Nothing logged yet." />
        ) : (
          rows.map((row, rowIdx) => (
